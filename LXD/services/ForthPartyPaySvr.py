...
class ForthPaySvr:
    ...
    def __init__(self):
        ...
        self.webserver.add_routes([web.post('/020pay', self.notify_handler_020)])
    ...
    async def notify_handler_020(self, request):
        data = await request.post()
        kstr = data['actual_price'] + data['bill_no'] + data['orderid'] + data['orderuid'] + data['price'] + config.pay_token
        if hashlib.md5(kstr.encode('utf-8')).hexdigest() == data['key']:
            ...
            try:
                await self.__QQbot__.send_private_msg_rate_limited(user_id=int(data['orderuid']), message='您的' + repr(float(data['price']) / 100) + '元充值已到账！')
