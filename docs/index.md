...
- ***setPassword*** `('设置密码', )`  
  ...
  为所有人设置自己的密码。类似于unix的passwd命令，向机器人发送“设置密码”后按提示操作即可。
...
- ***ask*** `('问',)`  
  ...
  现在增加一种私密问题，通过`addPrivateQuestion`来修改，无论在哪里询问，答案都会私发。
...
- ***addPrivateQuestion*** `('设置私密问题', '私密设问')`  
  `管理员`  `不需要@或私聊`
  
  可以使用这个功能来设置需要私聊回答的问题，格式为：
  ```
  addPrivateQuestion #问题# 答案
  ```
  答案可以是任意的QQ消息，可以是图文，也可以是多行消息，只要后面跟着就没问题。
...
- ***getBankStatementByInterval*** `('查询流水', '查账')`  
  ...
  根据命令行提示，输入起始日期和结束日期，这些时间可以是过去的，也可以是未来的。
  总之你要提供一个区间，系统将返回从起始日期的0点到结束日期的0点之间的所有账目，
  回复示例如下：
  ```
  您选择的区间内的账目如下：
  |序号：1 | 李迎雪赞助 | 40.0元|
  |序号：2 | 天上掉的 | 40.0元|
  |序号：3 | 自动记账-充值收入916327225 | 0.01元|
  区间总收入：80.01元
  ```
